...
import { CompleteProfileToast } from '@package/components';
...
import { CookieModal } from '@package/components';
...
class App extends Component {
   ...
   */
  componentDidMount() {
    if (!Number(cookie.load('confirm_ga'))) {
  ...
  render() {
    ...
    return (
      <Fragment>
        ...
        {this.props.gaConsent && <GoogleAnalytics />}
        <Header pathname={path} isEditMode={isCmsUI} />
...
export default compose(
  ...
  connect(
    (state, props) => ({
      ...
      token: state.userSession.token,
      content: state.content.data,
      gaConsent: state.cookieConsent.googleAnalytics,
      apiError: state.apierror.error,
      connectionRefused: state.apierror.connectionRefused,
    }),
    null,
